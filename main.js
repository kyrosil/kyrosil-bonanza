...
const rewards = [
    { id: 1, nameKey: 'reward1_name' },
    { id: 2, nameKey: 'reward2_name' },
    { id: 3, nameKey: 'reward3_name' }
];
...
window.addEventListener('load', () => {
        ...
        for (let col = 0; col < 6; col++) {
            ...
            for (let row = 0; row < 5; row++) {
                ...
                if (newGridSymbols[index] !== null) {
                    survivors.push(newGridSymbols[index]);
                }
            }
            ...
            for (let row = 0; row < 5; row++) {
                const index = row * 6 + col;
                newGridSymbols[index] = newColumn[row];
            }
        }
    ...
    async function handleNormalSpin() {
        ...
        if (scatterCount >= 4) {
            ...
            // Butonları aktif etmiyoruz, modal'ın cevabını bekliyoruz.
        } else {
            isSpinning = false;
            setButtonsState(true);
        }
    ...
    function attachEventListeners() {
        ...
        allElements.startBonusButton.addEventListener('click', () => {
            ...
            setButtonsState(true); // Butonları aktif et ki bonus fonksiyonu çalışabilsin
        ...
        allElements.marketButton.addEventListener('click', () => {
            if (playerData.username) {
                localStorage.setItem('languageStrings', JSON.stringify(languageStrings));
                window.location.href = `market.html?user=${playerData.username}&lang=${currentLanguage}`;
            } else {
